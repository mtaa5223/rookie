...
float3 ComputeFogScatteringColor(float3 worldPos)
{
    ...
    // Directions - New
    float3 viewDir = (_WorldSpaceCameraPos - worldPos) * -1.0;
    ...
    float sunRise = saturate(dot(float3(0.0, 500.0, 0.0), _Azure_SunDirection) / r);
    float moonRise = saturate(dot(float3(0.0, 500.0, 0.0), _Azure_MoonDirection) / r);
    float mieDepth = saturate(lerp(distance(_WorldSpaceCameraPos, mul((float3x3)_Azure_UpDirectionMatrix, worldPos.xyz)) * _ProjectionParams.w * 4.0, 1.0, _Azure_MieDepth));
    ...
    float3 BmTheta  = Pi14  * _Azure_Mie * miePhase * _Azure_MieColor.rgb * sunRise * mieDepth;
    ...
    BmTheta  = Pi14  * _Azure_Mie * miePhase * _Azure_MieColor.rgb * moonRise * mieDepth;
