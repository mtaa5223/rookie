    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 fragment_program (Varyings Input) : SV_Target
            {
                ...
                float3 Esun = _Azure_ScatteringMode == 0 ? lerp(fex, (1.0 - fex), sunset) : _Azure_ScatteringColor.rgb;
                ...
                float3 BmTheta  = Pi14  * _Azure_Mie * miePhase * _Azure_MieColor.rgb * sunRise;
                ...
                BmTheta  = Pi14  * _Azure_Mie * miePhase * _Azure_MieColor.rgb * moonRise;
                BrmTheta = (BrTheta + BmTheta) / (_Azure_Rayleigh + _Azure_Mie);
                Esun = _Azure_ScatteringMode == 0 ? (1.0 - fex) : _Azure_ScatteringColor.rgb;
                ...
                float3 skyLuminance = BrmTheta * _Azure_ScatteringColor.rgb * _Azure_Luminance * (1.0 - fex);

                // Sun texture
                float3 sunTexture = tex2D( _Azure_SunTexture, Input.SunPos + 0.5).rgb * _Azure_SunTextureColor.rgb * _Azure_SunTextureIntensity;
                ...
                if(iSphere(rayOrigin, rayDirection, moonPosition, 17370.0, normalDirection))
                {
                    ...
                    moonColor = moonTexture.rgb * moonSphere * _Azure_MoonTextureColor.rgb * _Azure_MoonTextureIntensity * moonExtinction;
                }
                ...
                float3 starfield = (stars + milkyWay) * _Azure_StarFieldColor.rgb * horizonExtinction * moonMask;
                ...
                float3 lowCloudColor = lerp(inScatter * _Azure_StaticCloudScattering * cloudExtinction, _Azure_StaticCloudColor.rgb, lowCloudTex.r * pow(fex.r, _Azure_StaticCloudExtinction)) * _Azure_StaticCloudOpacity;
                float3 higCloudColor = lerp(inScatter * _Azure_StaticCloudScattering * cloudExtinction, _Azure_StaticCloudColor.rgb, higCloudTex.r * pow(fex.r, _Azure_StaticCloudExtinction)) * _Azure_StaticCloudOpacity;
